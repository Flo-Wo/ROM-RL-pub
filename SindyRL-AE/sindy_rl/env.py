...
class BaseSurrogateEnv(gymnasium.Env):
    ...
    def __init__(self, config):
        ...
        # needed for the PDE controlgym real reward
        self.time_idx = 0
    ...
    def _init_act(self):
        ...
        default_act_bounds = np.full((self.act_dim, 2), [-np.inf, np.inf]).T

        act_bounds = self.config.get("act_bounds", None)

        if act_bounds is None:
            self.act_bounds = default_act_bounds
            self.action_space = Box(
                low=-np.inf * np.ones(self.act_dim), high=np.inf * np.ones(self.act_dim)
            )
        else:
            self.act_bounds = np.array(act_bounds, dtype=float).T
    ...
    def _init_obs(self):
        ...
        default_obs_bounds = np.full(
            (self.obs_dim, 2),
            [-10, 10],
        ).T
        obs_bounds = self.config.get("obs_bounds", None)

        if obs_bounds is None:
            self.obs_bounds = default_obs_bounds
            ...
            self.obs_bounds = np.array(obs_bounds, dtype=float).T
            ...
            self.observation_space = Box(
                low=np.float32(self.obs_bounds[0]), high=np.float32(self.obs_bounds[1])
            )
    ...
    def _init_dynamics_model(self):
        ...
        if self._init_weights and not (
            isinstance(self.dynamics_model, dynamics.AutoEncoderDynamicsModel)
        ):
    ...
    def step(self, action):
        ...
        proj_real_rew = 0
        if self.use_real_reward:
            rew = self.real_env.unwrapped.reward_class.reward_uVec_without_time(
                next_obs.reshape(21, 21, 2),
                self.time_idx,
                self.real_env.unwrapped.U_ref,
                action,
                self.real_env.unwrapped.action_ref,
            )
