...
class DynaSINDy(BaseDynaSINDy):
    ...
    def __init__(self, config, logger=None, init_drl=True, only_eval: bool = False):
        ...
        # additional flag to skip the reward fitting, if the real reward is used
        self.use_real_reward = self.drl_config["environment"]["env_config"].get(
            "use_real_reward", False
        )
        self.use_real_env = self.drl_config["environment"]["env_config"].get(
            "use_real_env", False
        )
    ...
    def _init_dynamics_model(self):
        ...
        # pass arguments from global config to the DRL agent's config file
        self.dyn_config["config"]["log_dir"] = self.config["log_dir"]
        self.dyn_config["config"]["plot_dir"] = self.config["plot_dir"]
        self.dyn_config["config"]["dummy_logger"] = self.config["dummy_logger"]
    ...
    def fit_dynamics(self):
        ...
        if self.use_real_env:
            # no need to train the dynamics model
            self.logger.info("Skipped fitting dynamics due to using real env.")
            return {}
        ...
        _, train_info = self.dynamics_model.fit(x, u)

        self.logger.info("Dynamic Fitting")
        for key, value in train_info.items():
            self.logger.info("{}: {}".format(key, value))
        self.logger.info("End of Dynamic fitting")
        return train_info
    ...
    def fit_rew(self):
        ...
        if self.use_real_reward:
            self.logger.info("Skipped reward fitting due to usage of real reward.")
            return
        x_off, u_off, rew_off = self.off_policy_buffer.to_list()
        x_on, u_on, rew_on = self.on_policy_buffer.to_list()
        ...
        self.logger.info("Reward fitted.")
